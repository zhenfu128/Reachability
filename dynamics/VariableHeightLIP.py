...
class VHLIP:
    def __init__(self, x=[0, 0, 0, 0, 0, 0], uMin=np.array([-0.05, -0.05, 5]), uMax=np.array([0.05, 0.05, 15]), dMin=np.array([0.0, 0.0, 0.0, 0.0]), dMax=np.array([0.0, 0.0, 0.0, 0.0]), dims=6, uMode="min", dMode="max"):
    ...
    def dynamics(self, t, state, uOpt, dOpt):
        ...
        x1_dot[0] = state[1]
        x2_dot[0] = uOpt[2] * (state[0] - uOpt[0])
        x3_dot[0] = state[3]
        x4_dot[0] = uOpt[2] * (state[2] - uOpt[1])
        x5_dot[0] = state[5]
        x6_dot[0] = uOpt[2] * state[4] - self.g
    ...
    def opt_ctrl(self, t, state, spat_deriv):
        ...
        with hcl.if_(self.uMode == "min"):
            ...
            SumUU[0] = spat_deriv[1] * state[0] + spat_deriv[3] * state[2] + spat_deriv[5] * \
                state[4] - spat_deriv[1] * self.uMax[0] - \
                spat_deriv[3] * self.uMax[1]
            SumUL[0] = spat_deriv[1] * state[0] + spat_deriv[3] * state[2] + spat_deriv[5] * \
                state[4] - spat_deriv[1] * self.uMax[0] - \
                spat_deriv[3] * self.uMin[1]
            SumLU[0] = pat_deriv[1] * state[0] + spat_deriv[3] * state[2] + spat_deriv[5] * \
                state[4] - spat_deriv[1] * self.uMin[0] - \
                spat_deriv[3] * self.uMax[1]
            SumLL[0] = spat_deriv[1] * state[0] + spat_deriv[3] * state[2] + spat_deriv[5] * \
                state[4] - spat_deriv[1] * self.uMin[0] - \
                spat_deriv[3] * self.uMin[1]

            with hcl.if_(SumUU[0] > 0):
                SumUU[0] = Sum
